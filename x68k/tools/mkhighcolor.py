...
from mksprites import NES_RGB, decode_tile, parse_ca65_data, parse_label_bytes, title_bitmap
...
def resize_nearest(width: int, height: int, pixels: list[tuple[int, int, int]]) -> list[list[tuple[int, int, int]]]:
    return [[pixels[min(height - 1, (2 * y + 1) * height // (2 * HEIGHT)) * width
                    + min(width - 1, (2 * x + 1) * width // (2 * WIDTH))]
...
def title_assets(assets: Path, original_assets: Path):
    ...
    for y, text in ((123, "START"), (137, "CONTINUE"), (151, "OPTION")):
        draw_text(opened, sprites, 60, y, text, (255, 255, 255))
...
def write_c(path: Path, direct, fallback, eyes, eyes_fallback, logo_positions, logo_colors) -> None:
