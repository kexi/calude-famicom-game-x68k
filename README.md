...
- **ゲーム背景**：16bit版では全4面に65536色モードの山と空を追加。ステージ開始画面も右端まで同じ多色背景を使い、台詞と顔の演出を重ねます。ゲーム中の遠景は固定表示で、地形・コイン・キャラクターは従来のパレット描画とスクロールを保ちます。4bit版では従来の背景とラウンド画面へ戻し、エンディングは両モードとも16色です。
...
ゲーム画面（CoreS3実機、320×240）。山と空は65536色モード、キャラクターと地形は従来のパレット描画です。

![原作風の4bitゲーム画面（CoreS3実機、320×240）](docs/screenshots/game-4bit-cores3.png)

4bit版では従来のドット絵・BG背景で遊べます。右側のゲーム領域外も従来どおりで、16bit版の遠景は表示しません。
