...
**実機での描画時間は未計測。** 書き込み時点で実機が応答しなくなっている
(繰り返しシリアル接続で何度か起きている症状)。dirty 画素が 2,810 分の 1 に
なったからといって描画時間がその比で縮むとは限らない
(タイル単位の粒度は 16x16 なので、1画素の変更でも 1 タイル分は描き直す)。
実測するまで「30fps に届く」とは書かない。
