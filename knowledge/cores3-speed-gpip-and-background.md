...
**ただし間隔を下げるだけでは 30fps にならない。** `completed()` は
次の開始を `max(前回開始 + minInterval, 前回終了 + 実測コスト x 1.5)`
で決める。40% の時間予算を返済させる仕組みで、描画コストの側が効く。
...
**30fps には描画そのものを 13.3ms 以下にする必要がある。** 現状 27.5ms なので
約半分。簡略背景は CPU を減らしたが描画時間は 27.5ms のまま変わっていない
(遠景の読み出しはゲスト側の 68000 の仕事で、ここでいう描画時間は
ESP32 側の LCD 合成の時間なので別物)。

間隔を 33,333us へ下げるビルドは焼いたが、実機が応答しなくなり計測できて
いない。上の計算から「下げるだけでは足りない」と分かったので既定へ戻した。
