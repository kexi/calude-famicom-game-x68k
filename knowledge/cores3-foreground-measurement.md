...
description: 実機で約7分プレイし計測。ゲーム中CPU約3.9MHz、描画28.4ms/frame・7.3fps。音声不足60.8%はCPU速度の10MHz比と恒等的に一致し、音声側に直すべきバグは無い。JIT無効下。
status: draft
...
1. **JITを有効にして同じ計測を取り直す**。今回の値はJIT無しの基準。
   ...
   上の恒等関係から、JITでCPUがX MHzになれば不足率は自動的に
   (1 - X/10)×100 % になると予測できる。この予測が外れたら、
   そのときこそ音声側に別の損失があることになる。
