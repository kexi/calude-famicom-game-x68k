...
* [ゲスト1フレームの内訳。94.8%が描画](guest-frame-profile.md) - video_present 71.4% + put_sprites 23.4%。physics/logicは合計2.4%。ゲスト予算の265%を使っており30fpsには5.3分の1が要る。

* [描画27.5→4.2msでfps 7.73→15.84](cores3-30fps-blocker.md) - GVRAM書込がdamage_.all()を呼んで差分描画を殺していた。座標単位のdirtyでタイルが300→30に。ブロック終端の79.5%は分岐/復帰で構造的に減らせず、JIT側での残り1.26倍は難しいと判明。
